...
$cities = User::where('role', 'lawyer')->where('is_approved', true)->pluck('city')->unique()->sort()->values();
...
$lawyer = auth()->user();
        ...
        $unreadNotifications = $lawyer->notifications()->where('is_read', false)->orderByDesc('created_at')->get();

        return view('lawyer.dashboard', compact('lawyer', 'appointments','totalAppointments','pendingAppointments','approvedAppointments', 'completedAppointments','unreadNotifications'
        ));
        }
